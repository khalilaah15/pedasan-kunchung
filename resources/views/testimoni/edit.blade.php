...
<div class="testimoni-container">
    <div class="testimoni-form-container">
        <div class="testimoni-header">
            <div>
                <h1 class="testimoni-title">Edit Testimoni</h1>
                <p class="testimoni-subtitle">Perbarui testimoni Anda</p>
            </div>
        </div>

        <div class="testimoni-form">
            <form id="testimoni-form" action="{{ route('testimoni.update', $testimoni->id) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="form-group">
                    ...
                    <div class="rating-container">
                        @for($i = 1; $i <= 5; $i++)
                            <button type="button" class="rating-btn" data-rating="{{ $i }}">
                                <svg class="rating-star-large rating-star {{ $i <= $testimoni->rating ? 'active' : 'inactive' }}" 
                    ...
                    <input type="hidden" name="rating" id="rating-input" value="{{ $testimoni->rating }}" required>
                    <div id="rating-text" style="font-size: 0.875rem; color: #6b7280; margin-top: 4px;">
                        @php
                            $ratingMessages = [
                                1 => 'Tidak Puas',
                                2 => 'Kurang Puas', 
                        ...
                        @endphp
                        {{ $ratingMessages[$testimoni->rating] }}
                    </div>
                    @error('rating')
                ...
                <div class="form-group">
                    ...
                    <textarea name="pesan" id="pesan" class="form-textarea" placeholder="Ceritakan pengalaman Anda menggunakan produk/layanan kami..." required>{{ old('pesan', $testimoni->pesan) }}</textarea>
                    <div class="char-count">
                        <span>Minimal 10 karakter</span>
                        <span id="char-count">{{ strlen($testimoni->pesan) }}/500</span>
                    </div>
                    @error('pesan')
                        <div style="color: #ef4444; font-size: 0.875rem; margin-top: 4px;">{{ $message }}</div>
                    @enderror
